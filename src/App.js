...
function LoginPage() {
  return (
    <div style={{ minHeight: '100vh', background: '#0d0d0d', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#1a1a1a', border: '1px solid #cc0000', borderRadius: '12px', padding: '48px', maxWidth: '400px', width: '100%', textAlign: 'center', boxShadow: '0 0 40px rgba(204,0,0,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '8px' }}>
          ...
          <span style={{ fontSize: '20px', fontWeight: 700, fontFamily: 'serif', color: '#fff' }}>The Economic Times</span>
        </div>
...
function NewsApp() {
  ...
  const handleFollowUp = async (q) => {
    ...
    try {
      const a = await callAI(`Answer this finance question in 4-5 sentences. Do not start with any intro phrase, jump straight into the answer: ${q}`, 400);
      setResult(a);
  ...
  return (
    <div style={{ background: '#0d0d0d', minHeight: '100vh', color: '#e0e0e0' }}>
      ...
      {/* Breaking News */}
      <div style={{ background: 'linear-gradient(90deg,#cc0000,#ff2200)', padding: '7px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <span style={{ background: '#fff', color: '#cc0000', fontWeight: 800, fontSize: '10px', padding: '2px 8px', borderRadius: '3px', letterSpacing: '1px', flexShrink: 0, animation: 'pulse 1.5s infinite' }}>LIVE</span>
        <span style={{ color: '#fff', fontSize: '13px', fontWeight: 600, fontFamily: 'sans-serif' }}>🔴 {BREAKING[breakingIdx]}</span>
        <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
      </div>

      {/* Header */}
      <div style={{ background: '#111', borderBottom: '3px solid #cc0000', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 20px rgba(204,0,0,0.3)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <input type="text" placeholder="Search news..." value={topic}
            ...
            style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: '6px', padding: '7px 12px', color: '#e0e0e0', fontSize: '13px', outline: 'none', width: '180px', fontFamily: 'sans-serif' }}
          />
          <button onClick={handleSearch} style={{ background: 'linear-gradient(135deg,#cc0000,#ff2200)', color: '#fff', border: 'none', borderRadius: '6px', padding: '7px 16px', cursor: 'pointer', fontSize: '13px', fontWeight: 700, boxShadow: '0 2px 10px rgba(204,0,0,0.4)' }}>
            ⚡ Brief Me
          </button>
          <button onClick={() => setShowBookmarks(!showBookmarks)} style={{ background: showBookmarks ? '#cc0000' : 'transparent', border: '1px solid #cc0000', color: showBookmarks ? '#fff' : '#cc0000', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
            🔖 {bookmarks.length}
          </button>
          {user?.imageUrl && <img src={user.imageUrl} alt={user.firstName} style={{ width: 32, height: 32, borderRadius: '50%', border: '2px solid #cc0000' }} />}
          <span style={{ color: '#e0e0e0', fontSize: '13px', fontWeight: 600 }}>{user?.firstName}</span>
          <SignOutButton>
            <button style={{ background: 'transparent', border: '1px solid #444', color: '#aaa', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontSize: '12px' }}>Sign Out</button>
          </SignOutButton>
        </div>
      </div>

      {/* Nav */}
      <div style={{ background: '#161616', borderBottom: '1px solid #2a2a2a', padding: '0 20px', display: 'flex', gap: 0, overflowX: 'auto' }}>
        {NAV_LINKS.map(nav => (
      ...
      {/* Indices Bar */}
      <div style={{ background: '#0f0f0f', borderBottom: '1px solid #1f1f1f', padding: '8px 20px', display: 'flex', gap: '28px', overflowX: 'auto', alignItems: 'center' }}>
        {indices.map((idx, i) => (
          <div key={i} style={{ flexShrink: 0 }}>
            <div style={{ fontSize: '10px', color: '#666', fontFamily: 'sans-serif' }}>{idx.name}</div>
            <div style={{ display: 'flex', gap: '6px', alignItems: 'baseline' }}>
              <span style={{ fontSize: '13px', fontWeight: 700, fontFamily: 'monospace', color: '#e0e0e0' }}>{idx.price.toLocaleString('en-IN')}</span>
              <span style={{ fontSize: '10px', color: idx.change >= 0 ? '#00ff88' : '#ff4444', fontWeight: 600 }}>
            ...
            <>
              ...
              {followUps.length > 0 && (
                <div style={{ marginBottom: '14px' }}>
                  ...
                  {followUps.map((q, i) => (
                    <button key={i} onClick={() => handleFollowUp(q)} style={{ background: 'transparent', border: '1px solid #cc000066', color: '#cc0000', borderRadius: '20px', padding: '4px 12px', cursor: 'pointer', fontSize: '11px', marginLeft: '6px', fontFamily: 'sans-serif', marginTop: '4px' }}>{q}</button>
                  ))}
                </div>
              )}
              <textarea
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
              {displayArticles.map((article, i) => (
                <div key={i}
                  style={{ background: '#1a1a1a', borderRadius: '10px', overflow: 'hidden', border: '1px solid #2a2a2a', transition: 'all 0.3s', cursor: 'pointer' }}
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 8px 30px rgba(204,0,0,0.2)'; e.currentTarget.style.borderColor = '#cc000055'; }}
                ...
                >
                  ...
                  <div style={{ padding: '14px' }}>
                    <h3 style={{ fontSize: '13px', fontWeight: 700, lineHeight: '1.6', marginBottom: '8px', color: '#e0e0e0', fontFamily: 'Georgia, serif' }}>{article.title}</h3>
                    {article.description && (
                      <p style={{ fontSize: '11px', color: '#777', lineHeight: '1.6', marginBottom: '12px', fontFamily: 'sans-serif' }}>{article.description}</p>
                    )}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <button onClick={() => toggleBookmark(article)} style={{ background: 'none', border: `1px solid ${isBookmarked(article) ? '#ffaa00' : '#333'}`, borderRadius: '4px', padding: '3px 8px', cursor: 'pointer', fontSize: '10px', color: isBookmarked(article) ? '#ffaa00' : '#666', fontFamily: 'sans-serif' }}>
                        {isBookmarked(article) ? '🔖 Saved' : '🔖'}
                      </button>
                      <button onClick={() => handleShare(article, i)} style={{ background: 'none', border: '1px solid #333', borderRadius: '4px', padding: '3px 8px', cursor: 'pointer', fontSize: '10px', color: '#666', fontFamily: 'sans-serif' }}>
                        {copiedIndex === i ? '✓' : '↗'}
                      </button>
                    </div>
                  </div>
